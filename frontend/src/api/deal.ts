...
export async function saveBuyerInfo(form: {
  ...
  const response = await apiRequest<BuyerInfoResponse>('/api/deal/buyer-info', {
    method: 'POST',
...
export async function fetchPropertyPreview(url: string, sessionId?: string): Promise<PropertyPreview> {
  ...
  const response = await apiRequest<ApiPropertyPreview>('/api/deal/property-info', {
    method: 'POST',
    body: JSON.stringify(payload),
  })
...
export async function fetchDealCheckResult(
  ...
  const response = await apiRequest<CheckRisksResponse>(
    `/api/deal/check-risks?session_id=${encodeURIComponent(resolvedSessionId)}`,
    {
      method: 'POST',
    },
  )
